...
type DisplayMemory = { id: string; familyId: string; title: string; description: string; memoryDate: string; photos: string[]; demo: boolean };
...
export function MemoriesView() {
  ...
  const albums: DisplayMemory[] = [
    ...live.map((memory) => ({ id: memory.id, familyId: memory.familyId, title: memory.title, description: memory.description, memoryDate: memory.memoryDate, photos: memory.photos, demo: false })),
    ...memories.map((memory, index) => ({ id: memory.id, familyId: "", title: memory.title, description: demoDescriptions[index] ?? "A preserved family memory.", memoryDate: memory.uploadedAt, photos: [memory.image, demoExtras[index % demoExtras.length], demoExtras[(index + 1) % demoExtras.length]], demo: true })),
  ];
  ...
  return <section>
    <div className="mb-12"><SectionTitle title="Memories"><div className="flex flex-wrap gap-2"><div ref={filterRef} className="relative"><button type="button" onClick={() => setFilterOpen((open) => !open)} className="surface inline-flex min-h-12 min-w-56 items-center justify-center gap-2 rounded-md px-5 text-sm font-semibold"><Filter className="size-4" />{filterDate ? formatLongDate(filterDate) : "All memories"}<ChevronDown className={`size-4 transition-transform ${filterOpen ? "rotate-180" : ""}`} /></button>{filterOpen && <div className="surface absolute right-0 top-full z-20 mt-2 w-56 rounded-xl p-2"><button type="button" onClick={() => { setFilterDate(""); setFilterOpen(false); }} className="w-full rounded-lg px-4 py-3 text-left text-sm font-semibold hover:bg-primary/5">All memories</button><button type="button" onClick={() => { setFilterOpen(false); setFilterCalendarOpen(true); }} className="w-full rounded-lg px-4 py-3 text-left text-sm font-semibold hover:bg-primary/5">Pick a date</button></div>}</div><Button primary disabled={!familyId} onClick={() => setFormOpen(true)}><Plus className="size-4" />New Memory</Button></div></SectionTitle>{error && <p role="alert" className="mt-3 text-sm text-destructive">{error}</p>}</div>
    ...
    {selected && <MemoryDetails memory={selected} onClose={() => setSelected(null)} onUpdated={(memory) => { const updated = { id: memory.id, familyId: memory.familyId, title: memory.title, description: memory.description, memoryDate: memory.memoryDate, photos: memory.photos, demo: false }; setLive((current) => current.map((item) => item.id === memory.id ? memory : item)); setSelected(updated); }} />}
    {filterCalendarOpen && <Modal onClose={() => setFilterCalendarOpen(false)}><div className="flex items-center justify-between"><h2 className="text-xl font-semibold">Filter by date</h2><button type="button" onClick={() => setFilterCalendarOpen(false)} aria-label="Close"><X /></button></div><div className="mt-5"><DatePicker value={filterDate} onChange={(value) => { setFilterDate(value); setFilterCalendarOpen(false); }} embedded /></div></Modal>}
  </section>;
}
